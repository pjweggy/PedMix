...
#if 0
    pair<int,int> array [ listPairs.size()  ];
    for(int i=0; i< (int)listPairs.size() ; ++i)
    {
        array[i] = listPairs[i];
    }
    qsort( (void *)array, listPairs.size(), sizeof( pair<int,int>  ), QSortCompareIntPair );
    // Now write back
    for(int i=0; i< (int)listPairs.size(); ++i)
    {
        listPairs[i] = array[i];
...
bool convert_index_to_vector(bool enum_already_set,int color_num, int box_num, int index, vector_t &result,vector_vector_t & enumeration)
{
	...
	if(enum_already_set) {
		...
		{
			for(int k = 0; k < enumeration[index].size();k++){
...
bool convert_vector_to_index(bool enum_already_set,vector_t query_vec,int &result_index,vector_vector_t & enumeration)
{
	...
	for(int i=0; i< query_vec.size();i++)box_num+=query_vec[i];
	vector_t vec;
	bool find = false;
